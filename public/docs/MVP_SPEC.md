...
- **模型选择**：
  ...
  - 人像主力：Crystal 4x（15 积分，长边≤1000px）
  - 印刷专业：Recraft（300DPI印刷级输出）
  - VIP极限：Crystal 10x（$3.99/张，长边≤1000px，不走积分）
- **增强参数**：
  - 放大倍率：2x, 4x
  - 噪声控制：低/中/高
  - 细节强度：轻度/标准/强力
...
2. **积分包（按需购买）：**
   ...
   | 积分包 | 价格 | 有效期 | 单价 | 性价比提升 |
   |--------|------|--------|------|-----------|
   | 100 积分 | $5.99 | 90 天 | $0.059/积分 | 基准 |
   | 200 积分 | $9.99 | 90 天 | $0.050/积分 | 省 15% |
   | 500 积分 | $19.99 | 180 天 | $0.040/积分 | 省 32% |
   | 1000 积分 | $29.99 | 180 天 | $0.030/积分 | 省 49% |

**月订阅（自动续费，用完不累积）：**

   | 订阅 | 月费 | 积分/月 | 年付 | 单价 |
   |------|------|---------|------|------|
   | Pro | $7.99 | 200 | $5.99/月 | $0.04/积分 |
   | Max | $14.99 | 500 | $11.99/月 | $0.03/积分 |
   | Ultra | $24.99 | 1000 | $19.99/月 | $0.025/积分 |

**积分消耗规则：**

| 档位 | 模型 | 积分 | API成本 | 说明 |
|------|------|------|---------|------|
| 免费档 | Real-ESRGAN | 1 | $0.002 | 无门槛体验，带人脸增强，拉新引流 |
| 付费基础 | Google Upscaler | 3 | $0.02 | 效果更好，价格适中 |
| 人像主力 | Crystal 4x | 15 | $0.10-0.20 | 4x，长边≤1000px |
| 印刷专业 | Recraft | 6 | $0.006 | $6/千张，印刷级输出 |
| VIP极限 | Crystal 10x | **$3.99/张** | $0.40-1.60 | 10x，长边≤1000px，**不走积分** |
...
| Recraft AI | 订阅制 $12+/月 | ~$0.45+ | ✅ 我们按张付费，更便宜 |
...
- **计费方式**：
  - Crystal 4x → 15 积分/张（走积分制）
  - Crystal 10x → **$3.99/张（不走积分，直接付费）**
- **原图输入限制（必须严格执行）**：
...
|------|------|------|------|-----------|--------|
| Crystal 4x | 4x | 15 积分 | $0.45 | $0.10~0.20 | **17%~58%** |
| Crystal 10x | 10x | **$3.99/张** | $3.99 | $0.40~1.60 | **20%~80%** |

> Crystal 10x 不走积分制，单独定价 $3.99/张。积分制无法覆盖 $1.60 的 API 成本。

**为什么 10x 不走积分**：
- $1.60 的 API 成本，按 $0.02/积分需要 80 积分/张
- 80 积分一张图 = 1000 积分包只能用 12 次，用户体验极差
- 单独定价 $3.99/张更清晰：高端功能，按次付费
...
| 原图尺寸 | 4x 输出 | 输出MP | API成本 | 收入(15积分) | 利润 |
|---------|---------|--------|---------|------------|------|
| 1000×1000 | 4000×4000 | 16MP | $0.20 | $0.45 | +$0.07 ✅ |
| 1000×750 | 4000×3000 | 12MP | $0.20 | $0.45 | +$0.07 ✅ |
| 1000×500 | 4000×2000 | 8MP | $0.10 | $0.45 | +$0.14 ✅ |
| 500×500 | 2000×2000 | 4MP | $0.05 | $0.45 | +$0.19 ✅ |

Crystal 10x（长边 ≤ 1000px，$3.99/张）：

| 原图尺寸 | 10x 输出 | 输出MP | API成本 | 收入 | 利润 |
|---------|----------|--------|---------|------|------|
| 1000×1000 | 10000×10000 | 100MP | $1.60 | $3.99 | +$0.39 ✅ |
| 1000×750 | 10000×7500 | 75MP | $1.60 | $3.99 | +$0.39 ✅ |
| 1000×500 | 10000×5000 | 50MP | $0.80 | $3.99 | +$1.19 ✅ |
| 500×500 | 5000×5000 | 25MP | $0.40 | $3.99 | +$1.59 ✅ |
...
function selectModel(user: User, image: Image): Model {
  ...
  if (user.modelPreference) {
    const creditMap = {
      'realesrgan': 1,
      'google-upscaler': 3,
      'crystal': 12,      // 4x = 15积分；10x 不走积分，$3.99/张
      'recraft': 6,
    }
...
// 10x 付了 $3.99 → 4x 只需 15 积分($0.45) → 退还 $1.75 或等值积分
{
  "success": true,
  "imageUrl": "https://...",
  "creditsUsed": 0,
...
- Crystal 4x: ✓ 运行正常 (99.2% uptime) - 15积分/张 - 长边≤1000px - 人像专精
- Recraft: ✓ 运行正常 (99.0% uptime) - 6积分/张 - $0.006/张超高性价比
- Crystal 10x: ⚠️ $3.99/张不走积分 - 长边≤1000px - 成本$0.40-1.60 - 毛利率20-80%
```
